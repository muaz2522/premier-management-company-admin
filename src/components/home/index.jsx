...
function HomeLayout() {
    return (
        <section>
            <div className="grid relative sm:grid-cols-[2fr_1.1fr] custom-min:grid-cols-1 custom-min:p-8 custom-md:p-16 custom-md:px-16">
                ...
                <p className="custom-min:pt-8 sm:pt-0 w-3/4 text-text-color flex items-center text-pretty">
                    ...
                    <div className="absolute right-32 bottom-10 border py-3 px-2 rounded-full">
                        {BelowArrowIcon && <BelowArrowIcon/>}
                    </div>
                    <div className="absolute -bottom-24 left-1/2 w-24 z-[-1] h-16 bg-[#DEEFFF] custom-shadow ">
                    </div>
            </div>
            <img src={src} alt="image" className="z-10 w-full h-[40rem] object-cover" />
            <Planning />
            <Strategic />
        </section>
    )
